import { clearAuthTokens, getRefreshToken, isMockApiEnabled, postToApi, setAuthTokens } from './api.js';
...
export async function refreshToken() {
  ...
  const response = await postToApi('/auth/refresh', storedRefreshToken ? { refreshToken: storedRefreshToken } : undefined, (mockApi) => mockApi.refreshToken(), {
    retryOnUnauthorized: false,
    headers: storedRefreshToken ? { Authorization: `Bearer ${storedRefreshToken}` } : undefined,
  });
