...
function normalizeJokerMode(source = {}) {
  ...
  // FEI is evaluated first so a newly reopened Joker state wins over any
  // stale ZAI flags returned by an older server or cached match snapshot.
  if (
    truthyFlag(source?.fei)
    || truthyFlag(source?.isFei)
    || truthyFlag(source?.isFEI)
    || truthyFlag(source?.feiActive)
    // Backward compatibility with the previous CHAI payload.
    || truthyFlag(source?.chai)
    || truthyFlag(source?.isChai)
    || truthyFlag(source?.chaiActive)
    || truthyFlag(nested?.fei)
    || truthyFlag(nested?.isFei)
    || truthyFlag(nested?.chai)
    || truthyFlag(nested?.isChai)
    || ['fei', 'chai', 'joker_on', 'open_joker', 'reopen_joker', 'wild_on'].includes(mode)
  ) return 'fei';
  ...
  if (
    ...
    || truthyFlag(source?.zaiTriggeredByFaceOne)
    ...
    || ['zai', 'zai_locked', 'zai_active', 'ones_locked', 'joker_off', 'joker_still_off', 'no_joker', 'no_jokers', 'wild_off', 'wild_still_off'].includes(mode)
  ) return 'zai';

  return 'normal';
}

function getFeiQuantityStep() {
  // Official client rules are fixed at +2. The backend Patch 1 exposes the
  // same value; stale room settings must not silently change the UI rule.
  return OFFICIAL_FEI_QUANTITY_STEP;
}
...
function getBidJokerTag(bid = {}) {
  ...
  if (mode === 'zai' || Number(bid?.face) === 1) return 'ZAI';
  if (mode === 'fei') return 'FEI';
  return '';
}
...
function getMatchJokerDisplay(match = {}, bid = null, tx = (value) => value) {
  ...
  const mode = Number(bid?.face ?? matchBid?.face) === 1 && normalizeJokerMode(source) !== 'fei'
    ? 'zai'
    : normalizeJokerMode(source);

  if (mode === 'zai') return { mode: 'zai', label: tx('ZAI'), detail: tx('Joker OFF'), className: 'is-zai', jokerWildActive: false };
  if (mode === 'fei') return { mode: 'fei', label: tx('FEI'), detail: tx('Joker ON'), className: 'is-fei', jokerWildActive: true };
...
function buildBidJokerPayload({ currentBid, selectedQuantity, selectedFace, zaiEnabled, feiEnabled = false, match }) {
  ...
  const jokerMode = fei ? 'fei' : zai ? 'zai' : zaiInherited ? 'zai_locked' : 'normal';
  const jokerWildActive = jokerMode === 'normal' || jokerMode === 'fei';

  return {
    zai,
    isZai: zai,
    zaiDeclared: explicitZai,
    zaiTriggeredByFaceOne: faceOneTriggersZai,
    zaiInherited,
    fei,
    isFei: fei,
    // Legacy aliases during deployment.
    chai: fei,
    isChai: fei,
    feiAutoApplied: false,
    feiReopensJoker: fei,
    jokerMode,
    jokerWildActive,
  };
...
function getSelectedBidJokerInfo({ currentBid, selectedQuantity, selectedFace, zaiEnabled, feiEnabled = false, match, tx = (value) => value }) {
  const payload = buildBidJokerPayload({ currentBid, selectedQuantity, selectedFace, zaiEnabled, feiEnabled, match });
  if (payload.fei) return { ...payload, label: tx('FEI'), detail: tx('Joker ON'), className: 'is-fei' };
...
function describeLastAction(match, tx) {
  ...
  if (action.type === 'zai' || action.type === 'call_zai' || action.type === 'declare_zai') return `${actor} ${tx('called ZAI')}`;
  if (action.type === 'fei' || action.type === 'call_fei' || action.type === 'declare_fei') return `${actor} ${tx('called FEI')}`;
...
function specialActionSubtitle({
  actionMode,
  selection,
  validation,
  currentBid,
  ...
  if (!canSubmit && hasServerActionRules) return tx('ZAI is not available for this bid');
  if (validation.valid) return `${selection.quantity} x ${selection.face} · ${tx('Joker OFF')}`;
  if (validation.code === 'EXPLICIT_ZAI_ON_FACE_ONE') {
    return tx('Use Confirm Bid — Face 1 activates ZAI automatically');
  }
  if (validation.code === 'ZAI_BLOCKED_AFTER_FACE_ONE') {
    return tx('ZAI cannot be used after face 1 was called');
  }
  if (validation.code === 'OPENING_ZAI_TOO_LOW') {
    return `${tx('Opening ZAI minimum')} ${validation.minimumQuantity} x ${selection.face}`;
  }
  if (validation.code === 'ZAI_LOWER_THAN_CURRENT') {
    return tx('ZAI must repeat or raise the current bid');
  }
  if (validation.code === 'ZAI_ALREADY_ACTIVE') return tx('ZAI is already active');
  return tx('ZAI is not available for this bid');
}
...
export default function Gameplay({ navigation, data, backendActions, backendStatus, i18n }) {
  ...
  const currentBidJokerMode = currentBidJokerInfo?.mode || 'normal';
  ...
  const [feiAnimating, setFeiAnimating] = useState(false);
  ...
  const canAct = Boolean(currentMatchId && match && !isFinished && !isOpeningCoinFlipActive && myTurn && !viewerEliminated && !isBusy && !isTurnIntroPlaying && !slamAnimating && !callLiarAnimating && !zaiAnimating && !feiAnimating);
  ...
  const serverAdvertisesFei = feiActionAliases.some((action) => availableActions.includes(action))
    || truthyFlag(match?.bidControls?.feiAvailable)
    || truthyFlag(match?.bidControls?.canDeclareFei)
    || truthyFlag(match?.bidRules?.feiAvailable)
    || truthyFlag(match?.bidRules?.canDeclareFei);
  const serverDisablesFei = feiActionAliases.some((action) => disabledActions.includes(action));
  const canSubmitFei = canAct && (!hasServerActionRules || serverAdvertisesFei) && !serverDisablesFei;
  ...
  const [feiEnabled, setFeiEnabled] = useState(false);
  ...
  const submitZaiBid = () => {
    ...
    if (!canSubmitFei || !feiSelectionValidation.valid) return;
    const actionPayload = {
      matchId: currentMatchId,
      type: 'fei',
      bid: {
  ...
  const clearFeiTimers = () => {
    if (typeof window !== 'undefined') {
      feiTimersRef.current.forEach((timerId) => window.clearTimeout(timerId));
    }
  ...
  const feiSelectionValidation = validateOfficialFeiSelection({
    ...
    currentMode: currentBidJokerMode,
    totalDice: quantityMax,
    feiEnabled: getBidControls(match)?.feiEnabled !== false,
    quantityStep: getFeiQuantityStep(match),
  });
  const zaiSelectionValidation = validateOfficialZaiSelection({
    currentBid,
    quantity: directZaiBid.quantity,
    face: directZaiBid.face,
    currentMode: currentBidJokerMode,
    faceOneTriggeredZaiThisRound: faceOneTriggeredZaiThisRound(match),
    totalDice: quantityMax,
    openingMinimum: getOpeningMinimumQuantity(match, tablePlayerCount, directZaiBid.face),
    zaiEnabled: getBidControls(match)?.zaiEnabled !== false,
  });
  const feiAvailable = Boolean(isFeiActionMode && feiSelectionValidation.valid);
  const zaiAvailable = Boolean(!isFeiActionMode && zaiSelectionValidation.valid);
  const specialActionAvailable = isFeiActionMode ? feiAvailable : zaiAvailable;
  ...
  const selectedBidJokerInfo = getSelectedBidJokerInfo({ currentBid, selectedQuantity, selectedFace, zaiEnabled, feiEnabled, match, tx });
  const directZaiSubtitle = specialActionSubtitle({
    actionMode: specialActionMode,
    selection: directZaiBid,
    validation: isFeiActionMode ? feiSelectionValidation : zaiSelectionValidation,
    currentBid,
    totalDice: quantityMax,
    canSubmit: isFeiActionMode ? canSubmitFei : canSubmitZai,
    hasServerActionRules,
    tx,
  });
  ...
  const zaiDisabled = isFeiActionMode
    ? (!canSubmitFei || !specialActionAvailable || !coinBetIsValid)
    : (!canSubmitZai || !specialActionAvailable || !coinBetIsValid);
  ...
  return (
    ...
    >
      ...
      <div className="gameplay-bid-selector gameplay-bid-selector--casino-v2">
            ...
            {isDiceFacePickerOpen ? (
              <div className="gameplay-bid-selector__faceDialRing" role="listbox" aria-label={tx('Dice face')}>
                {diceFacePickerOptions.map((value, index) => {
                  ...
                  // During an active bid, every face remains selectable because
                  // the player may still raise quantity. The action buttons show
                  // whether the exact visible selection is legal.
                  const disabled = currentBid ? false : !normalBidFaceAllowed && !openingFaceAllowed;
                  return (
                    <button
                      key={`face-${value}`}
                      type="button"
        ...
        <div className="gameplay-bid-selector__actionRow" role="group" aria-label={tx('Bid actions')}>
          ...
          <button
            ...
            aria-pressed={false}
            disabled={zaiDisabled || zaiAnimating || feiAnimating}
          >
      ...
      <FeiAnimationOverlay active={feiAnimating} runId={feiAnimationRun} />
