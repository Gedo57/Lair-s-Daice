  ...
  return (
    <section className="screen special-event-screen" aria-label={tx('Special Event')}>
      ...
      <div className="special-event-cards" aria-label={tx('Event cards')}>
        {eventCards.map((event) => <EventCard key={event.key} event={event} onPlay={backendActions?.playSpecialEvent} tx={tx} />)}
      </div>
      ...
      <div className="special-event-missions">
        ...
        {missions.map((mission) => <Mission key={mission.key} mission={mission} tx={tx} />)}
      </div>
